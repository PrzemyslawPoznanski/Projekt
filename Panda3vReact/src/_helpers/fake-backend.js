        ...
        return new Promise((resolve, reject) => {
                ...
                if (users.find(x => x.email === user.email)) {
                    ...
                    setTimeout(() => {
                        alertService.info(`
                            ...
                            <div><strong>NOTE:</strong> Real api would send real email.</div>
                        `, { autoClose: false });
                    }, 1000);
            ...
            function getUsers() {
                if (!isAuthorized(Role.Admin)) return unauthorized();
                    var data = users;
                    var wantedData = data.filter(function(requestedAttribute) {
                        return requestedAttribute.role == "Admin" || requestedAttribute.role == "User";
                    })
            ...
            function getEvents() {
                    ...
                    var data = users;
                    var wantedData = data.filter(function(requestedAttribute) {
                        return requestedAttribute.role == "Verified" || requestedAttribute.role == "Not Verified";
                    })
                return ok(wantedData);
            }
